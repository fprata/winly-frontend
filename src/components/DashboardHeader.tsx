  ...
  return (
    <header className="h-14 bg-white border-b border-zinc-200 flex items-center justify-between px-6 sticky top-0 z-30">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-medium text-zinc-400">{t('systemOperational')}</span>
      </div>

      <div className="flex items-center gap-3">
        <LanguageSwitcher />

        <Link href="/matches" className="relative p-1.5 rounded-lg hover:bg-zinc-100 text-zinc-400 hover:text-zinc-600 transition-all">
          <Bell size={18} />
          {matchCount > 0 && (
            <span className="absolute top-1 right-1 bg-blue-600 ring-2 ring-white text-white text-[9px] font-bold rounded-full w-3.5 h-3.5 flex items-center justify-center">
