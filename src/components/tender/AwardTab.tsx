          ...
          <div>
            ...
            {tender.winners_list && tender.winners_list.length > 0 ? (
              tender.winners_list.map((winner: any, i: number) => (
                <Link key={i} href={`/intelligence/competitors?name=${encodeURIComponent(winner.winner_name)}&backUrl=${encodeURIComponent('/tenders/' + tenderId)}`} className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-amber-200 transition-all group mb-2">
