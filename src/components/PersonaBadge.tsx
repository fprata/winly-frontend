...
export function PersonaBadge({ personaName }: PersonaBadgeProps) {
  ...
  if (!key) {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-gray-900 text-white">
        {personaName}
      </span>
    );
  }
    ...
    <div className="relative inline-block group">
      <div className="flex items-center gap-1.5">
        ...
        <button 
          ...
          className="w-4 h-4 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 hover:bg-teal-100 hover:text-teal-600 transition-colors"
        >
          <Info size={10} />
        </button>
      </div>
      ...
      {isOpen && (
        <>
          ...
          ></div>
          <div className="absolute left-0 top-full mt-2 w-64 bg-gray-900 text-white p-4 rounded-xl shadow-2xl z-50 animate-in fade-in zoom-in duration-200">
