...
import { Search as SearchIcon, TrendingUp, Users, ArrowUpRight, ShieldCheck, Trophy, ArrowLeft, Building2, Check } from 'lucide-react';
      ...
      <div className="mb-10">
        ...
        <div className="flex items-center gap-4 mt-2">
          ...
          <Link href="/personas" className="text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-1">
            {t('competitorPersonasGuide')} <ArrowUpRight size={14} />
          </Link>
        </div>
      </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('nameLabel', { defaultValue: 'Company Name' })}</th>
                    ...
                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-40 text-right">{t('stats.totalWins')}</th>
                    ...
                    searchResults.map((c, i) => (
                      ...
                      >
                        <td className="px-6 py-5">
                          <p className="font-bold text-slate-800 group-hover:text-blue-700 transition-colors">{c.name}</p>
                        </td>
                        <td className="px-6 py-5">
                          <span className="px-2 py-1 bg-slate-100 text-slate-500 rounded text-[10px] font-black uppercase">{c.country}</span>
                        </td>
                        <td className="px-6 py-5 text-right font-black text-slate-700">
                          {c.total_wins?.toLocaleString()}
                        </td>
